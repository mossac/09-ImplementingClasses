...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        x = abs(previousx) - abs(self.x)
        y = abs(previousy) - abs(self.y)
        self.distance = self.distance + ((x ** 2) + (y ** 2)) ** (1 / 2)
    ...
    def get_distance_from(self,other):
        x=abs(other.x)-abs(self.x)
        y=abs(other.y)-abs(self.y)
        return ((x**2)+(y**2))**(1/2)
    def get_distance_from_start(self):
        x = abs(self.startx) - abs(self.x)
        y = abs(self.starty) - abs(self.y)
        return ((x ** 2) + (y ** 2)) ** (1 / 2)
    def get_distance_traveled(self):
        return self.distance
    def closer_to(self,p1, p2):
        x1 = abs(p1.x) - abs(self.x)
        y1 = abs(p1.y) - abs(self.y)
        x2 = abs(p2.x) - abs(self.x)
        y2 = abs(p2.y) - abs(self.y)
        if ((x1 ** 2) + (y1 ** 2)) ** (1 / 2) < ((x2 ** 2) + (y2 ** 2)) ** (1 / 2):
            return p1
        if ((x2 ** 2) + (y2 ** 2)) ** (1 / 2) < ((x1 ** 2) + (y1 ** 2)) ** (1 / 2):
            return p2
        else:
            return p1
